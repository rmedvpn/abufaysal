...
namespace Faysal.Helpers
{
    ...
    "
        ...
        public static bool Login3(string username, string password, bool rememberMe = false)
        {
            var db = Database.Open("faysal");

            var user = db.QuerySingleOrDefault(@"
        SELECT u.UserId, u.UserName, u.SessionVersion, m.Password
        FROM UserProfile u
        INNER JOIN webpages_Membership m ON u.UserId = m.UserId
        WHERE u.UserName = @0", username);
        ...
        new Claim(ClaimTypes.NameIdentifier, u_id.ToString()),
        new Claim(ClaimTypes.Name, user.UserName.ToString()),
        new Claim("sv", sessionVersion.ToString()) // <-- add session version
    }, CookieAuthenticationDefaults.AuthenticationScheme);
            ...
            _http.HttpContext.User = principal; // make CurrentUserId valid immediately
            return true;
        }


        public static bool LoginBCK(string username, string password, bool rememberMe = false)
        {
            var db = Database.Open("faysal");

            var user = db.QuerySingleOrDefault(@"
        SELECT u.UserId, u.UserName, m.Password 
        FROM UserProfile u 
        INNER JOIN webpages_Membership m ON u.UserId = m.UserId 
        WHERE u.UserName = @0", username);
            ...
            AppFunctions.WriteDebugLine("USER: " + user.UserName);
            AppFunctions.WriteDebugLine("storedPassword: " + storedPassword);
            AppFunctions.WriteDebugLine("password: " + password);
        ...
        new Claim(ClaimTypes.Name, user.UserName.ToString())
            ...
            // IMPORTANT: Only set ExpiresUtc when rememberMe is true, and use UTC *offset*
            ...
            // Avoid .Wait() (can deadlock); block safely if you must keep a sync method
            _http!.HttpContext!.SignInAsync(
                CookieAuthenticationDefaults.AuthenticationScheme,
                principal,
                authProps
            ...
            // Make the claims available in THIS request so CurrentUserId != 0 immediately
        ...
        new Claim(ClaimTypes.Name, user.UserName.ToString())
            ...
            _http!.HttpContext!.SignInAsync(CookieAuthenticationDefaults.AuthenticationScheme, principal, authProps).Wait();
            return true;
        }
